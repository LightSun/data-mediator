...
import static com.heaven7.java.data.mediator.support.SupportUtils.*;
...
public abstract class TypeHandler {
    ...
    private static class PrimitiveTypeHandler extends TypeHandler {
        ...
        @Override
        public void read(JsonReader in, GsonProperty property, Object t) throws IOException {
            Object val = readPrimitiveOrItsBox(in, property);
            setValue(property, t, val);
        }
    }
        ...
        public void read(JsonReader in, GsonProperty property, Object t) throws IOException {
            ...
            if (simpleType.isPrimitive() || isBoxedClass(simpleType)) {
                while (in.hasNext()) {
                    ...
                    sa.put(key, readPrimitiveOrItsBox(in, property));
                }
            }else{
                TypeAdapter adapter = getTypeAdapter(simpleType);
                while (in.hasNext()) {
    ...
    private static class ArrayTypeHandler extends TypeHandler {
        ...
        public void read(JsonReader in, GsonProperty property, Object t) throws IOException {
            ...
            if (simpleType.isPrimitive() || isBoxedClass(simpleType)) {
                while (in.hasNext()){
                    list.add(readPrimitiveOrItsBox(in, property));
                }
            }else{
                TypeAdapter adapter = getTypeAdapter(simpleType);
                while (in.hasNext()){
